...
%% Time analysis
    function deltaT=time()
        ...
        deltaT=vid.Duration./vid.NumberofFrames;
        deltaT = deltaT./3600; %transfer unit into hr
    end
%% Detect Objects
...
% current frame, and update its bounding box accordingly.
    ...
    function predictNewLocationsOfTracks()
        for i = 1:length(tracks)
            ...
            Centroid = predict(tracks(i).kalmanFilter);
            
            % shift the bounding box so that its center is at 
            % the predicted location
            predictedCentroid = int32(Centroid) - bbox(3:4) / 2;
            %distance between current and predicted location
            dis=sqrt(abs(Centroid(1)-double(predictedCentroid(1)).^2+abs(Centroid(2)-double(predictedCentroid(2)).^2)));
            % transfer the unit from pixel to miles approximately
            dis=dis./675840;
            tracks(i).velocity = (dis./deltaT).*10;
    ...
    function displayTrackingResults()
        ...
        if ~isempty(tracks)
            ...
            if ~isempty(reliableTracks)
                ...
                v = cat(1,reliableTracks.velocity);
                avgV= sum(v)./length(reliableTracks);
                ...
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                    ...
                    isPredicted(predictedTrackInds) = {'HighSpeedWarning'};
                    labels = strcat(labels, isPredicted);
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                ...
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                    [1 20 1 20], 'AvgSpeed:');
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                    [20 40 1 1], int2str(avgV),'Color',{'blue'});
                % draw on the mask
