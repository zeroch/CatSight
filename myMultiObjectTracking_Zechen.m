...
while ~isDone(obj.reader)
    ...
    frame = readFrame();
%     [centroids, bboxes, mask] = m_detectObjects(frame);
    [m_cen, m_bb, centroids, bboxes, mask] = m_detectObjects(frame);
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects()
        ...
        obj.reader = vision.VideoFileReader('viptraffic.avi');
...
%% Another method to detectObject;

    %% m_detectObject: function description
%     function [centroids, bboxes, mask] = m_detectObjects(frame)
    function [m_centroids, m_bboxes, centroids, bboxes, mask] = m_detectObjects(frame);
        % outputs = ;
        %% centroids : double   size 2  2 
        %% bboxes : int32       size 2  4
        mask = MOGmask(obj.detector, frame, 2, 1);
        [~, m_centroids, m_bboxes] = obj.blobAnalyser.step(mask);

        CC = bwconncomp(mask);            % Gets all the different objects in the binary image
        box_data = regionprops(CC);    % Gets properties (Bounding Box,Centroid,etc) of each object
        [object_size, ~] = size(box_data);
        if object_size == 0
            centroids = [];
            bboxes = [];
            return;
        ...
        for object_count = 1: object_size
            centroids(object_count,:) = int32(box_data(object_count,1).Centroid);
            bboxes(object_count,:) = int32(box_data(object_count,1).BoundingBox);
        end
    end
